...
def test_ingestion_chunks_embeds_and_preserves_provenance() -> None:
    engine = ResearchIntelligence(chunk_size=300, chunk_overlap=50); workspace_id = uuid4()
    document = engine.ingest_text(title="Research memo", text=("Evidence about portfolio construction and risk controls. " * 40), uri="https://example.test/memo", publisher="Example Research", workspace_id=workspace_id)
    provenance = engine.provenance(document.document_id); chunks = provenance["chunks"]
...
def test_vector_search_is_ranked_and_synthesis_cites_exact_chunks() -> None:
    ...
    hits = engine.search("risk exposure margin", limit=5); synthesis = engine.synthesize("risk exposure margin", limit=5)
...
def test_research_api_is_registered_and_operational() -> None:
    client = TestClient(app); workspace_id = str(uuid4())
    payload = {"workspace_id": workspace_id, "title": "API source", "text": "Vector search should retrieve this evidence about factor models.", "uri": "https://example.test/factors"}
    response = client.post("/api/research/documents", json=payload); assert response.status_code == 201; document = response.json()
    search = client.post("/api/research/search", json={"workspace_id": workspace_id, "query": "factor models", "limit": 5}); assert search.status_code == 200; assert search.json()[0]["document"]["document_id"] == document["document_id"]
    synthesis = client.post("/api/research/synthesize", json={"workspace_id": workspace_id, "query": "factor models", "limit": 5}); assert synthesis.status_code == 200; assert synthesis.json()["citations"]
