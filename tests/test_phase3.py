...
def test_workspace_manager_rejects_invalid_limit() -> None:
    with __import__("pytest").raises(ValueError): WorkspaceManager(max_concurrent=0)
...
def test_knowledge_search_is_ranked_and_validated() -> None:
    ...
    with __import__("pytest").raises(ValueError): kb.search(" ")
    for limit in (0, 101):
        with __import__("pytest").raises(ValueError): kb.search("momentum", limit=limit)
...
def test_api_exposes_product_workspace_and_artifact_flow() -> None:
    ...
    artifact = client.post(f"/api/workspaces/{workspace['workspace_id']}/artifacts", json={"kind": "research", "producer": "quant_researcher", "content": {"factor": "momentum"}})
...
def test_api_knowledge_and_safe_tool_registry() -> None:
    ...
    response = client.post("/api/knowledge", json={"workspace_id": workspace_id, "title": "Walk forward", "text": "out of sample validation", "source": "internal", "tags": ["validation"]})
    assert response.status_code == 201
    results = client.get("/api/knowledge/search", params={"q": "validation", "workspace_id": workspace_id}).json()
...
def test_platform_repository_persists_and_lists_metadata() -> None:
    ...
    rows = {"workspace": workspace.model_dump(mode="json"), "artifact": artifact.model_dump(mode="json")}
    repository = PlatformRepository(lambda: FakeConnection())
    assert repository.create_workspace(workspace) == workspace
    assert PlatformRepository(lambda: FakeConnection(many=[rows["workspace"]])).list_workspaces()[0].workspace_id == workspace.workspace_id
    artifact_repo = PlatformRepository(lambda: FakeConnection(one={"version": 0}))
    assert artifact_repo.put_artifact(artifact).version == 1
    assert PlatformRepository(lambda: FakeConnection(many=[rows["artifact"]])).list_artifacts(workspace.workspace_id)[0].artifact_id == artifact.artifact_id
...
def test_platform_repository_rejects_empty_dsn_and_connects_lazily() -> None:
    with __import__("pytest").raises(ValueError): connection_factory_from_dsn(" ")
    factory = connection_factory_from_dsn("postgresql://example")
    with patch("psycopg.connect") as connect:
        sentinel = object(); connect.return_value = sentinel
...
def test_platform_repository_raises_on_missing_return_rows() -> None:
    workspace_id = uuid4()
    artifact = ArtifactRecord(workspace_id=workspace_id, kind="report", producer="qa", content={"x": 1}, content_hash="0" * 64)
    repository = PlatformRepository(lambda: FakeConnection(one=None))
    with __import__("pytest").raises(RuntimeError, match="artifact version"): repository.put_artifact(artifact)
    document = KnowledgeDocument(title="x", text="y", source="z")
    with __import__("pytest").raises(RuntimeError, match="knowledge upsert"): repository.upsert_knowledge(document)
