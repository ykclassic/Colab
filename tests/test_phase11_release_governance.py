...
def test_manifest_is_reproducible_and_changes_when_inputs_change() -> None:
    first = ReproducibilityManifest(
        dataset_checksum="d", feature_config={"window": 5}, strategy_parameters={"x": 1}, code_revision="c", dependency_lock_hash="l"
    )
    second = ReproducibilityManifest(
        dataset_checksum="d", feature_config={"window": 6}, strategy_parameters={"x": 1}, code_revision="c", dependency_lock_hash="l"
    )
    assert first.manifest_hash != second.manifest_hash
    assert first.manifest_hash == ReproducibilityManifest.model_validate(first.model_dump()).manifest_hash


def test_versions_are_immutable_and_unique() -> None:
    ...
    with pytest.raises(GovernanceError):
        governance.register_version(StrategyVersion(version_id=uuid4(), name=first.name, version=first.version, artifact_digest="other", manifest_hash=first.manifest_hash))
...
def test_regression_suite_passes_within_tolerance_and_fails_critical_case() -> None:
    suite = RegressionSuite(
        "core",
        (
            RegressionCase(name="metric", expected=10, tolerance=0.1),
            RegressionCase(name="critical", expected=1, tolerance=0, critical=True),
        ),
    )
