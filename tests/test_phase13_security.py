...
def test_workspace_http_error_paths_and_secure_listing(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    archived_again = client.post(f"/api/workspaces/{workspace_id}/archive", json={"version": archived.json()["version"]}, headers=headers)
    assert archived_again.status_code == 200
    restored = client.post(f"/api/workspaces/{workspace_id}/restore", json={"version": archived_again.json()["version"]}, headers=headers)
