...
from .background_job_handlers import research_ingestion, report_generation

def _backtest(payload:dict[str,Any], progress:Any)->dict[str,Any]:
    from .quant_api import QuantRunRequest, _dataset, signal
    r=QuantRunRequest.model_validate(payload); progress(15,"Preparing market dataset"); dataset=_dataset(r); features=FeatureEngineer().build(dataset,(r.fast_window,r.slow_window)); progress(55,"Running backtest")
    result=QuantitativeResearchLab().backtest(dataset,features,signal,{"fast_window":r.fast_window,"slow_window":r.slow_window},initial_capital=r.initial_capital); progress(90,"Finalizing backtest artifact")
    return {"workspace_id":str(r.workspace_id),"dataset_checksum":dataset.checksum,"features":list(features.feature_names),"result":result.model_dump(mode="json")}
