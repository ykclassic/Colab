...
class BackgroundJob(BaseModel):
    ...
    job_id: UUID; workspace_id: UUID; workflow_id: UUID|None=None; job_type: str; idempotency_key: str
    status: str=JobStatus.QUEUED; payload: dict[str,Any]=Field(default_factory=dict); progress:int=Field(0,ge=0,le=100)
    progress_message:str|None=None; attempt:int=Field(0,ge=0); max_attempts:int=Field(3,ge=1,le=20)
    lease_owner:str|None=None; lease_expires_at:datetime|None=None; artifact_id:UUID|None=None
    result:dict[str,Any]|None=None; error:str|None=None; notification_status:str="pending"; notification_error:str|None=None
    created_at:datetime; started_at:datetime|None=None; completed_at:datetime|None=None; updated_at:datetime
...
class InMemoryJobQueue:
    ...
    jobs:dict[UUID,BackgroundJob]=field(default_factory=dict); idempotency:dict[tuple[UUID,str],UUID]=field(default_factory=dict)
    events:list[JobEvent]=field(default_factory=list); artifacts:dict[UUID,JobArtifact]=field(default_factory=dict)
    def enqueue(self,workspace_id:UUID,job_type:str,payload:dict[str,Any],idempotency_key:str,workflow_id:UUID|None=None,max_attempts:int=3)->BackgroundJob:
        ...
        now=datetime.now(UTC); job=BackgroundJob(job_id=uuid4(),workspace_id=workspace_id,workflow_id=workflow_id,job_type=job_type,idempotency_key=idempotency_key,payload=payload,max_attempts=max_attempts,created_at=now,updated_at=now)
        self.jobs[job.job_id]=job; self.idempotency[key]=job.job_id; self._event(job,0,"Job queued"); return job
    ...
    def claim(self,worker_id:str)->BackgroundJob|None:
        ...
        return None
    def heartbeat(self,job_id:UUID,worker_id:str)->BackgroundJob:self._owner(job_id,worker_id); j=self.jobs[job_id]; j.lease_expires_at=datetime.now(UTC)+timedelta(seconds=self.lease_seconds); return j
    ...
    def fail(self,job_id:UUID,worker_id:str,error:str)->BackgroundJob:
        self._owner(job_id,worker_id); j=self.jobs[job_id]; j.error=error[:5000]; j.lease_owner=None; j.lease_expires_at=None; j.updated_at=datetime.now(UTC)
        j.status=JobStatus.QUEUED if j.attempt<j.max_attempts else JobStatus.FAILED; j.completed_at=None if j.status==JobStatus.QUEUED else datetime.now(UTC); self._event(j,j.progress,"Job retry scheduled" if j.status==JobStatus.QUEUED else "Job failed"); return j
