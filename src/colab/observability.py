...
def configure_telemetry(service_name: str = "colab-agent-platform") -> None:
    resource = Resource.create(
        {
            ...
            "service.version": os.getenv("COLAB_VERSION", "0.8.0"),
        }
    )
    ...
    try:
        ...
        _logger.debug("OpenTelemetry tracer provider already configured: %s", exc)
    try:
        metrics.set_meter_provider(MeterProvider(resource=resource))
    except (RuntimeError, ValueError) as exc:
        _logger.debug("OpenTelemetry meter provider already configured: %s", exc)
...
request_counter = meter.create_counter(
    "colab.http.requests", unit="1", description="HTTP requests"
)
request_latency = meter.create_histogram(
    "colab.http.duration", unit="ms", description="HTTP request latency"
)
job_counter = meter.create_counter(
    "colab.jobs.completed", unit="1", description="Completed or failed jobs"
)
job_latency = meter.create_histogram(
    "colab.jobs.duration", unit="ms", description="Job execution latency"
)
retry_counter = meter.create_counter(
    "colab.jobs.retries", unit="1", description="Job retries"
)
...
def instrument_boundary(name: str):
    def decorator(function: Any) -> Any:
        def wrapped(*args: Any, **kwargs: Any) -> Any:
            with span(
                name,
                attributes={"code.function": getattr(function, "__qualname__", name)},
            ):
