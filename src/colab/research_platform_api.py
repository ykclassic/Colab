...
class ResearchPlatformServices:
    def __init__(self, database_dsn: str | None = None, production: bool = False) -> None:
        ...
        if database_dsn:
            ...
            self.store = None
        if production and not __import__("os").getenv("OPENAI_API_KEY"):
            raise ResearchPlatformError("OPENAI_API_KEY is required in production for real embeddings")
        self.embedder = OpenAIEmbeddingProvider() if __import__("os").getenv("OPENAI_API_KEY") else DeterministicEmbeddingProvider()
        self.extractor = TextExtractionPipeline()
        self.engine = ResearchIntelligence(embedder=self.embedder)
...
def register_dataset(payload: DatasetCreate, request: Request) -> DatasetRecord:
    ...
    schema_hash = sha256(__import__("json").dumps(payload.schema, sort_keys=True).encode()).hexdigest()
    ...
    return _services(request).datasets.register(dataset)
...
def list_datasets(request: Request, workspace_id: UUID) -> list[DatasetRecord]:
    ...
    return _services(request).datasets.list(workspace_id)
...
def ingest(payload: IngestRequest, request: Request) -> ExtractionRecord:
    ...
    try:
        ...
        document = services.engine.ingest_text(title=payload.title, text=text, uri=payload.uri, workspace_id=payload.workspace_id, source_type="research-platform")
        ...
        if services.store is not None:
            for chunk in chunks:
                services.store.save_chunk(chunk_id=chunk.chunk_id, dataset_id=dataset.dataset_id, document_id=document.document_id,
                                          ordinal=chunk.ordinal, text=chunk.text, embedding=services.embedder.embed(chunk.text),
                                          content_hash=chunk.content_hash)
        extraction = ExtractionRecord(dataset_id=dataset.dataset_id, media_type=payload.media_type, filename=payload.filename,
                                      extractor=extractor_name, extracted_text_hash=sha256(text.encode()).hexdigest(), character_count=len(text))
        if services.store is not None:
...
def search(payload: SearchRequest, request: Request) -> list[dict[str, Any]]:
    ...
    if services.store is not None:
        hits = services.store.hybrid_search(workspace_id=payload.workspace_id, query=payload.query, embedding=query_vector,
                                            limit=payload.candidate_limit, candidate_limit=payload.candidate_limit)
        ...
        ranked = [RetrievalCandidate(chunk=h.chunk.chunk_id, document_id=h.document.document_id, source_id=h.source.source_id,
                                     text=h.chunk.text, title=h.source.title, uri=h.source.uri,
                                     vector_score=h.score, lexical_score=0.0) for h in hits]
...
def create_report(payload: ReportRequest, request: Request) -> dict[str, Any]:
    ...
    citations = [{"citation_id": x["citation_id"], "chunk_id": x["chunk_id"], "document_id": x["document_id"],
                  "dataset_id": x["dataset_id"], "title": x["title"], "uri": x["uri"], "quote": x["quote"]} for x in evidence]
    ...
    config = {"limit": payload.limit, "candidate_limit": payload.candidate_limit, "reranker": "deterministic-overlap-v1", "hybrid": True}
