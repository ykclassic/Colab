...
Phase 9 provides deterministic orchestration around bounded agents: parallel
execution, explicit delegation, typed agent-to-agent messages, conflict
resolution, arbitration, budgets, and auditable task state.  Agents receive
immutable workflow snapshots and cannot mutate canonical workflow state.
"""
...
class CollaborationEngine:
    ...
    def delegate(self, role: AgentRole, objective: str, stage: str, *, priority: int = 100,
                 dependencies: tuple[UUID, ...] = (), budget: int = 1) -> TaskSpec:
        if role not in self._agents:
            raise CollaborationError(f"no agent registered for role {role.value}")
        return TaskSpec(role=role, objective=objective, stage=stage, priority=priority,
                        dependencies=dependencies, budget=budget)

    @staticmethod
    def _ready(tasks: list[TaskSpec], completed: set[UUID]) -> list[TaskSpec]:
        return sorted(
            (task for task in tasks if task.task_id not in completed and
    ...
    def _run_one(self, task: TaskSpec, state: WorkflowState) -> TaskResult:
        ...
        try:
            ...
            return TaskResult(task_id=task.task_id, role=task.role, status=TaskStatus.SUCCEEDED,
                              artifacts=result.artifacts, notes=result.notes,
                              started_at=started, completed_at=datetime.now(UTC))
        except Exception as exc:
            return TaskResult(task_id=task.task_id, role=task.role, status=TaskStatus.FAILED,
    ...
    def execute_parallel(self, tasks: list[TaskSpec], state: WorkflowState) -> tuple[TaskResult, ...]:
        ...
        with ThreadPoolExecutor(max_workers=self._max_workers) as pool:
            while len(completed) < len(tasks):
                ...
                if failed:
                    blocked = [task for task in tasks if any(dep in {r.task_id for r in failed} for dep in task.dependencies)]
                    if blocked:
                        raise CollaborationError("dependency failed; downstream task cannot execute")
        return tuple(results)

    def send(self, sender: AgentRole, recipient: AgentRole, task_id: UUID, content: str,
             message_type: MessageType = MessageType.REQUEST,
             conversation_id: UUID | None = None) -> AgentMessage:
        ...
        message = AgentMessage(sender=sender, recipient=recipient, task_id=task_id,
    ...
    def detect_conflict(self, task_id: UUID, positions: dict[AgentRole, str], reason: str) -> Conflict:
        if len(positions) < 2:
    ...
    def arbitrate(self, conflict: Conflict, scorer: Callable[[AgentRole, str], float] | None = None) -> ArbitrationDecision:
        ...
        ranked = sorted(((score(role, position), role) for role, position in conflict.positions.items()),
        ...
        return ArbitrationDecision(conflict_id=conflict.conflict_id, winner=winner,
    ...
    def collaborate(self, tasks: list[TaskSpec], state: WorkflowState) -> CollaborationResult:
        ...
        if len(successful) > 1:
            positions = {result.role: str(result.artifacts[0].content) if result.artifacts else "no artifact"
                         for result in successful}
            if len(set(positions.values())) > 1:
                conflict = self.detect_conflict(successful[0].task_id, positions, "parallel outputs differ")
                decisions.append(self.arbitrate(conflict))
        return CollaborationResult(results=results, messages=self.messages(), decisions=tuple(decisions))
