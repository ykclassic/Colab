...
class WorkspaceCreate(BaseModel):
    ...
    name:str=Field(min_length=1,max_length=200); product_goal:str=Field(min_length=1,max_length=10000); priority:int=Field(100,ge=0,le=1000); strategies:list[StrategySpec]=Field(default_factory=list)
...
class ExecutionCreate(BaseModel):
    model_config=ConfigDict(extra="forbid")
    workflow_id:UUID; workspace_id:UUID; stage:str=Field(min_length=1,max_length=100); idempotency_key:str=Field(min_length=1,max_length=255); max_attempts:int=Field(3,ge=1,le=20)
...
class PlatformServices:
    def __init__(self,max_concurrent:int=2,database_dsn:str|None=None)->None:
        ...
        self.database:PostgresPlatformStore|None=None; lease_seconds=int(os.getenv("COLAB_EXECUTION_LEASE_SECONDS","300"))
        if dsn:
        ...
        else:self.workspaces=WorkspaceManager(max_concurrent=max_concurrent); self.artifacts=InMemoryArtifactStore(); self.knowledge=KnowledgeBase(); self.tools=ToolRegistry(); self.execution=ExecutionCoordinator(lease_seconds=lease_seconds); self.observability=ObservabilityRecorder()
...
_DASHBOARD_HTML="""<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Colab Agent Platform</title></head><body><h1>Colab Agent Platform</h1><p>Production multi-agent quantitative research platform.</p></body></html>"""
