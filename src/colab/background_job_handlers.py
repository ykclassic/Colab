...
def research_ingestion(payload:dict[str,Any], progress:Any)->dict[str,Any]:
    ...
    services=ResearchPlatformServices(database_dsn=payload.get("database_dsn"),production=bool(payload.get("production",False)))
    progress(10,"Extracting source content")
    text,extractor=services.extractor.extract(payload["content"].encode(),payload.get("media_type","text/plain"),payload.get("filename"))
    progress(35,"Chunking and embedding research")
    document=services.engine.ingest_text(title=payload["title"],text=text,uri=payload["uri"],workspace_id=UUID(payload["workspace_id"]),source_type="research-platform")
    services.documents[document.document_id]=UUID(payload["dataset_id"])
    ...
    progress(95,"Research artifact ready")
    return {"document_id":str(document.document_id),"dataset_id":payload["dataset_id"],"extraction":extraction.model_dump(mode="json"),"chunk_count":len(chunks)}
...
def report_generation(payload:dict[str,Any],progress:Any)->dict[str,Any]:
    ...
    services=ResearchPlatformServices(database_dsn=payload.get("database_dsn"),production=bool(payload.get("production",False)))
    progress(15,"Retrieving cited evidence")
    query=payload["query"]; limit=int(payload.get("limit",10)); candidate_limit=int(payload.get("candidate_limit",40)); vector=services.embedder.embed(query)
    hits=services.store.hybrid_search(workspace_id=UUID(payload["workspace_id"]),query=query,embedding=vector,limit=candidate_limit,candidate_limit=candidate_limit) if services.store else services.engine.search(query,limit=candidate_limit,workspace_id=UUID(payload["workspace_id"]))
    ranked=rerank(query,hits,limit); citations=[]; evidence={}
    for h in ranked:
    ...
    progress(60,"Validating citations")
    ...
    progress(95,"Report artifact ready"); return report.model_dump(mode="json")
