...
class PostgresWorkflowRepository:
    ...
    def create(self, state: WorkflowState) -> int:
        ...
        with self._connection() as conn, conn.transaction(), conn.cursor() as cur:
            ...
            self._append_event(cur, state, 1, None, state.current_stage.value, state.current_stage.value,
    ...
    def save(self, state: WorkflowState, expected_version: int) -> int:
        ...
        with self._connection() as conn, conn.transaction(), conn.cursor(row_factory=dict_row) as cur:
            cur.execute("SELECT current_stage FROM public.workflows WHERE workflow_id=%s AND version=%s FOR UPDATE",
            ...
            source_value = previous["current_stage"] if isinstance(previous, dict) else previous[0]
            source_stage = Stage(source_value)
            ...
            decision_offset = int(decision_row["count"] if isinstance(decision_row, dict) else decision_row[0])
            ...
            approval_offset = int(approval_row["count"] if isinstance(approval_row, dict) else approval_row[0])
            ...
            cur.execute("SELECT sequence_no,resulting_state_hash FROM public.workflow_events WHERE workflow_id=%s ORDER BY sequence_no DESC LIMIT 1",
                        (state.workflow_id,))
            last_event = cur.fetchone()
            if last_event is None:
                sequence, previous_hash = 1, None
            else:
                sequence = int(last_event["sequence_no"] if isinstance(last_event, dict) else last_event[0]) + 1
                previous_hash = last_event["resulting_state_hash"] if isinstance(last_event, dict) else last_event[1]
            event_type = "stage_advanced" if source_stage != state.current_stage else "workflow_snapshot_saved"
            self._append_event(cur, state, sequence, previous_hash, source_stage.value, state.current_stage.value,
                               digest, new_version, event_type, "orchestrator",
                               f"{source_stage.value} -> {state.current_stage.value}")
        return new_version

    def validate_integrity(self, workflow_id: UUID) -> None:
        """Replay the durable event/snapshot chain and raise on any tampering."""
        with self._connection() as conn, conn.cursor(row_factory=dict_row) as cur:
            cur.execute("""SELECT sequence_no,event_type,from_stage,to_stage,previous_state_hash,resulting_state_hash,
                          snapshot_version,actor,message,occurred_at
                   FROM public.workflow_events WHERE workflow_id=%s ORDER BY sequence_no""", (workflow_id,))
            events = cur.fetchall()
            if not events:
                raise WorkflowIntegrityError("workflow has no integrity events")
            validate_event_chain(events)
            cur.execute("SELECT version,state,state_hash FROM public.workflow_checkpoints WHERE workflow_id=%s ORDER BY version",
                        (workflow_id,))
    ...
    def _write_checkpoint(cur: Any, state: WorkflowState, version: int, payload: dict[str, Any], digest: str) -> None:
        cur.execute("INSERT INTO public.workflow_checkpoints (workflow_id,version,state,state_hash) VALUES (%s,%s,%s,%s)",
                    (state.workflow_id, version, payload, digest))

    @staticmethod
    def _append_event(cur: Any, state: WorkflowState, sequence: int, previous_hash: str | None,
                      from_stage: str, to_stage: str, resulting_hash: str, snapshot_version: int,
                      event_type: str, actor: str, message: str) -> None:
        cur.execute("""INSERT INTO public.workflow_events
    ...
    def _write_children(cur: Any, state: WorkflowState, decision_offset: int, approval_offset: int) -> None:
        for task in state.agent_tasks:
            cur.execute("""INSERT INTO public.agent_tasks (task_id,workflow_id,role,objective,stage,status)
                VALUES (%s,%s,%s,%s,%s,%s)
        ...
        for collection, artifacts in (("research", state.research_artifacts), ("strategy", state.strategy_candidates),
            ...
            for artifact in artifacts:
                cur.execute("""INSERT INTO public.artifacts
        ...
        for assessment in state.risk_assessments:
            cur.execute("""INSERT INTO public.risk_assessments
        ...
        for event in state.audit_events:
            ...
            cur.execute("""INSERT INTO public.audit_events
