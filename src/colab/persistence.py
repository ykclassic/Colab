...
class PostgresWorkflowRepository:
    ...
    The workflow ``version`` is an optimistic-concurrency token. Checkpoints are immutable
    snapshots; tasks are upserted; artifacts, risk assessments, decisions, approvals, and
    ...
    def create(self, state: WorkflowState) -> int:
        ...
        with self._connection() as conn, conn.transaction():
            with conn.cursor() as cur:
                ...
                self._write_children(cur, state, decision_offset=0, approval_offset=0)
                self._write_checkpoint(cur, state, 1, payload)
        return 1

    def load(self, workflow_id: UUID) -> tuple[WorkflowState, int]:
        """Load canonical state and its current optimistic-concurrency version."""
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    UPDATE public.workflows
                       SET schema_version = %s, product_goal = %s, product_brief = %s,
                           roadmap = %s, current_stage = %s, iteration_count = %s,
                           budgets = %s, final_package = %s, version = %s
    ...
    def _write_checkpoint(cur: Any, state: WorkflowState, version: int, payload: dict[str, Any]) -> None:
        cur.execute(
            "INSERT INTO public.workflow_checkpoints (workflow_id, version, state) VALUES (%s, %s, %s)",
    ...
    def _write_children(cur: Any, state: WorkflowState, decision_offset: int, approval_offset: int) -> None:
        ...
        collections = (
            ("research", state.research_artifacts), ("strategy", state.strategy_candidates),
    ...
    def _load_state(cur: Any, workflow: dict[str, Any]) -> WorkflowState:
        ...
        tasks = [AgentTask(task_id=r["task_id"], role=r["role"], objective=r["objective"],
                           stage=r["stage"], status=r["status"]) for r in cur.fetchall()]
        cur.execute("SELECT * FROM public.artifacts WHERE workflow_id = %s ORDER BY created_at", (workflow_id,))
        artifacts = [Artifact(artifact_id=r["artifact_id"], kind=r["kind"], version=r["version"],
                              producer=r["producer"], content=r["content"], created_at=r["created_at"])
        ...
        events = [AuditEvent(event_id=r["event_id"], event_type=r["event_type"], stage=r["stage"],
                             actor=r["actor"], message=r["message"], at=r["at"]) for r in cur.fetchall()]
        ...
        by_collection = {"research": [], "strategy": [], "implementation": [], "validation": []}
        for artifact, collection in zip(artifacts, self._artifact_collections(cur, workflow_id), strict=True):
            by_collection[collection].append(artifact)
        return WorkflowState(
            workflow_id=workflow_id, schema_version=workflow["schema_version"], product_goal=workflow["product_goal"],
            ...
            risk_assessments=risks, implementation_artifacts=by_collection["implementation"],
            validation_results=by_collection["validation"], decisions=decisions, approvals=approvals,
            audit_events=events, iteration_count=workflow["iteration_count"], budgets=workflow["budgets"],
            final_package=workflow["final_package"],
        )

    @staticmethod
    def _artifact_collections(cur: Any, workflow_id: UUID) -> list[str]:
        cur.execute("SELECT collection FROM public.artifacts WHERE workflow_id = %s ORDER BY created_at", (workflow_id,))
        return [str(r["collection"]) for r in cur.fetchall()]
