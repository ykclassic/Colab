...
def evaluate_results(results: list[EvaluationResult], agent_id: UUID) -> EvaluationSummary:
    ...
    if not items:
        raise AgentPlatformError("no evaluation results for agent")
    ...
    cost = sum(x.cost_usd for x in items)
...
def historical_performance(results: list[EvaluationResult], agent_id: UUID) -> HistoricalPerformance:
    ...
    if not items:
        raise AgentPlatformError("no historical results for agent")
    scores = [x.score for x in items]
    trend = (statistics.fmean(scores[-min(10, len(scores)):]) - statistics.fmean(scores[:min(10, len(scores))])) if len(scores) > 1 else 0.0
    passed = sum(x.passed for x in items)
    cost = sum(x.cost_usd for x in items)
...
def arbitrate(candidates: list[ArbitrationCandidate], minimum_evidence: float = .5) -> ArbitrationDecision:
    if not candidates:
        raise AgentPlatformError("candidates cannot be empty")
    scored: list[tuple[ArbitrationCandidate, float, float]] = []
    for candidate in candidates:
        evidence_score = statistics.fmean(x.strength for x in candidate.evidence) if candidate.evidence else 0.0
        score = candidate.confidence * (.5 + .5 * evidence_score)
        if evidence_score < minimum_evidence:
            score *= .5
        scored.append((candidate, score, evidence_score))
    scored.sort(key=lambda x: x[1], reverse=True)
    winner, score, evidence = scored[0]
    tied = len(scored) > 1 and math.isclose(score, scored[1][1], rel_tol=1e-9, abs_tol=1e-9)
    rationale = "Selected highest confidence weighted by independent evidence strength; weakly evidenced candidates are penalized."
    return ArbitrationDecision(winner_agent_id=winner.agent_id, score=score, evidence_score=evidence, rationale=rationale, tied=tied)
...
def retrieve_memory(memories: list[MemoryRecord], workspace_id: UUID, query: str, agent_id: UUID | None = None, limit: int = 10) -> list[MemoryRecord]:
    ...
    eligible = [m for m in memories if m.workspace_id == workspace_id and (agent_id is None or m.agent_id in (None, agent_id)) and (m.expires_at is None or m.expires_at > now)]
    ranked = sorted(eligible, key=lambda m: (len(terms & set((m.key + " " + m.value).lower().split())), m.confidence), reverse=True)
    return ranked[:limit]
...
def summarize_costs(records: list[CostRecord], agent_id: UUID) -> CostSummary:
    ...
    if not items:
        raise AgentPlatformError("no cost records for agent")
    tokens = sum(x.tokens_in + x.tokens_out for x in items)
    return CostSummary(agent_id=agent_id, calls=len(items), total_cost_usd=sum(x.cost_usd for x in items), tokens_in=sum(x.tokens_in for x in items), tokens_out=sum(x.tokens_out for x in items), mean_latency_ms=statistics.fmean(x.latency_ms for x in items), cost_per_1k_tokens=sum(x.cost_usd for x in items) / tokens * 1000 if tokens else 0.0)
