...
def historical_performance(results: list[EvaluationResult], agent_id: UUID) -> HistoricalPerformance:
    ...
    n = min(10, len(scores))
    trend = statistics.fmean(scores[-n:]) - statistics.fmean(scores[:n]) if len(scores) > 1 else 0.0
    passed = sum(x.passed for x in items)
    cost = sum(x.cost_usd for x in items)
...
def arbitrate(candidates: list[ArbitrationCandidate], minimum_evidence: float = .5) -> ArbitrationDecision:
    ...
    for candidate in candidates:
        evidence = statistics.fmean(x.strength for x in candidate.evidence) if candidate.evidence else 0.0
        score = candidate.confidence * (.5 + .5 * evidence)
        score *= .5 if evidence < minimum_evidence else 1.0
        scored.append((candidate, score, evidence))
