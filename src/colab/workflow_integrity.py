...
def canonical_state_payload(state: WorkflowState) -> dict[str, Any]:
    ...
    Audit events are intentionally excluded because an integrity event describes
    the state transition and including the event itself would create a cycle.
    """
...
def validate_stage_transition(source: Stage, target: Stage, state: WorkflowState) -> None:
    ...
    if target not in allowed[source]:
        raise WorkflowIntegrityError(
            f"invalid workflow transition: {source.value} -> {target.value}"
        )
    if source == Stage.RISK and target == Stage.IMPLEMENTATION:
        if not state.risk_assessments or state.risk_assessments[-1].decision != Decision.APPROVE:
            raise WorkflowIntegrityError("implementation requires an explicit approved risk assessment")
    if source == Stage.HUMAN_REVIEW and target == Stage.COMPLETE:
        if not state.approvals or state.approvals[-1].get("decision") != Decision.APPROVE:
...
def validate_event_chain(events: Iterable[dict[str, Any]]) -> None:
    ...
    for event in ordered:
        ...
        if sequence != expected_sequence:
            raise WorkflowIntegrityError(
                f"workflow event sequence gap: expected {expected_sequence}, got {sequence}"
            )
        ...
        if source != target:
            # Structural validation; gate-specific approval checks are performed
            # by the repository against the corresponding durable snapshot.
            allowed = {
                Stage.INTAKE: {Stage.DECOMPOSITION},
                Stage.DECOMPOSITION: {Stage.RESEARCH},
                Stage.RESEARCH: {Stage.STRATEGY},
                Stage.STRATEGY: {Stage.RISK},
                Stage.RISK: {Stage.IMPLEMENTATION, Stage.STRATEGY, Stage.REJECTED},
                Stage.IMPLEMENTATION: {Stage.VALIDATION},
                Stage.VALIDATION: {Stage.SYNTHESIS},
                Stage.SYNTHESIS: {Stage.HUMAN_REVIEW},
                Stage.HUMAN_REVIEW: {Stage.COMPLETE, Stage.REJECTED},
                Stage.COMPLETE: set(),
                Stage.REJECTED: set(),
            }
            if target not in allowed[source]:
                raise WorkflowIntegrityError(
                    f"workflow event {sequence} skips a stage: {source.value} -> {target.value}"
                )
...
def validate_snapshot_hash(snapshot: dict[str, Any]) -> None:
    ...
    if snapshot.get("state_hash") != expected:
        raise WorkflowIntegrityError(
            f"snapshot {snapshot.get('version', '?')} failed state hash validation"
        )
