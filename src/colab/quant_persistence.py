...
class PostgresQuantStore:
    ...
    def register_strategy(self, strategy: StrategyRecord) -> StrategyRecord:
        with self._connection_factory() as conn, conn.transaction(), conn.cursor(row_factory=dict_row) as cur:
            cur.execute("""INSERT INTO public.quant_strategies
                (strategy_id,workspace_id,name,version,code_revision,parameters,dataset_ids,created_at)
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
                (strategy.strategy_id, strategy.workspace_id, strategy.name, strategy.version,
                 strategy.code_revision, Jsonb(strategy.parameters), Jsonb([str(x) for x in strategy.dataset_ids]), strategy.created_at))
            row = cur.fetchone()
        if row is None:
    ...
    def save_experiment(self, record: ExperimentRecord, result: dict[str, Any], digest: str) -> ExperimentRecord:
        with self._connection_factory() as conn, conn.transaction(), conn.cursor(row_factory=dict_row) as cur:
            cur.execute("""INSERT INTO public.quant_experiments
                (experiment_id,workspace_id,strategy_id,strategy_version,dataset_ids,seed,configuration,metrics,result,experiment_hash,created_at)
