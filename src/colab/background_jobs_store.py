...
from .background_jobs import BackgroundJob, JobArtifact, JobEvent, JobStatus
...
class PostgresJobQueue:
    ...
    def enqueue(self,workspace_id:UUID,job_type:str,payload:dict[str,Any],idempotency_key:str,workflow_id:UUID|None=None,max_attempts:int=3)->BackgroundJob:
        with self.connection_factory() as c, c.transaction(), c.cursor() as x:
            x.execute("""INSERT INTO public.background_jobs(workspace_id,workflow_id,job_type,idempotency_key,payload,max_attempts) VALUES(%s,%s,%s,%s,%s,%s) ON CONFLICT(workspace_id,idempotency_key) DO NOTHING RETURNING *""",(workspace_id,workflow_id,job_type,idempotency_key,json.dumps(payload),max_attempts))
            row=x.fetchone()
            if row is None:
                x.execute("SELECT * FROM public.background_jobs WHERE workspace_id=%s AND idempotency_key=%s",(workspace_id,idempotency_key)); row=x.fetchone()
            return self._job(x,row)
    ...
    def list(self,workspace_id:UUID,limit:int=100)->list[BackgroundJob]:
        with self.connection_factory() as c, c.cursor() as x:
            x.execute("SELECT * FROM public.background_jobs WHERE workspace_id=%s ORDER BY created_at DESC LIMIT %s",(workspace_id,limit)); return [self._job(x,r) for r in x.fetchall()]
    def claim(self,worker_id:str)->BackgroundJob|None:
        with self.connection_factory() as c, c.transaction(), c.cursor() as x:
            x.execute("SELECT * FROM public.phase21g_claim_background_job(%s,%s)",(worker_id,self.lease_seconds)); row=x.fetchone()
    ...
    def progress(self,job_id:UUID,worker_id:str,value:int,message:str)->BackgroundJob:
        ...
        with self.connection_factory() as c,c.transaction(),c.cursor() as x:
            x.execute("UPDATE public.background_jobs SET progress=%s,progress_message=%s,updated_at=now() WHERE job_id=%s AND status='running' AND lease_owner=%s AND progress<=%s RETURNING *",(value,message,job_id,worker_id,value)); row=x.fetchone()
            if row is None:raise RuntimeError("job is not owned or progress moved backwards")
            x.execute("INSERT INTO public.background_job_events(job_id,workspace_id,status,progress,message) VALUES(%s,%s,'running',%s,%s)",(job_id,row[1],value,message)); return self._job(x,row)
    def complete(self,job_id:UUID,worker_id:str,result:dict[str,Any],kind:str)->BackgroundJob:
        ...
        with self.connection_factory() as c,c.transaction(),c.cursor() as x:
            ...
            x.execute("UPDATE public.background_jobs SET status='succeeded',progress=100,result=%s,artifact_id=%s,lease_owner=NULL,lease_expires_at=NULL,completed_at=now() WHERE job_id=%s RETURNING *",(raw,artifact_id,job_id)); row=x.fetchone(); x.execute("INSERT INTO public.background_job_events(job_id,workspace_id,status,progress,message,metadata) VALUES(%s,%s,'succeeded',100,'Job completed',%s)",(job_id,row[1],json.dumps({'artifact_id':str(artifact_id)}))); return self._job(x,row)
    ...
    def cancel(self,job_id:UUID)->BackgroundJob:
        with self.connection_factory() as c,c.transaction(),c.cursor() as x:
            ...
            if row is None:return self.get(job_id)
            return self._job(x,row)
    ...
    def artifact(self,job_id:UUID)->JobArtifact|None:
        with self.connection_factory() as c,c.cursor() as x:
            x.execute("SELECT artifact_id,job_id,workspace_id,kind,content,content_hash,created_at FROM public.background_job_artifacts WHERE job_id=%s ORDER BY created_at DESC LIMIT 1",(job_id,)); r=x.fetchone()
            return None if r is None else JobArtifact(artifact_id=r[0],job_id=r[1],workspace_id=r[2],kind=r[3],content=r[4],content_hash=r[5],created_at=r[6])
    def _job(self,x:Any,r:Any)->BackgroundJob:
        cols=['job_id','workspace_id','workflow_id','job_type','idempotency_key','status','payload','progress','progress_message','attempt','max_attempts','lease_owner','lease_expires_at','artifact_id','result','error','notification_status','notification_error','created_at','started_at','completed_at','updated_at']
        return BackgroundJob(**dict(zip(cols,r)))
