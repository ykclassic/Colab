...
class PostgresJobQueue:
 ...
 def enqueue(self,workspace_id:UUID,job_type:str,payload:dict[str,Any],idempotency_key:str,workflow_id:UUID|None=None,max_attempts:int=3)->BackgroundJob:
  with self.connection_factory() as c,c.transaction(),c.cursor() as x:
   ...
   if row is None:raise RuntimeError("failed to enqueue background job")
 ...
 def complete(self,job_id:UUID,worker_id:str,result:dict[str,Any],kind:str)->BackgroundJob:
  ...
  with self.connection_factory() as c,c.transaction(),c.cursor() as x:
   ...
   if row is None:raise PermissionError("worker does not own job lease")
   x.execute("INSERT INTO public.background_job_artifacts(job_id,workspace_id,kind,content,content_hash) VALUES(%s,%s,%s,%s,%s) ON CONFLICT(job_id,content_hash) DO UPDATE SET content=EXCLUDED.content RETURNING artifact_id",(job_id,row[1],kind,raw,digest)); artifact_row=x.fetchone()
   if artifact_row is None:raise RuntimeError("failed to persist job artifact")
   artifact_id=artifact_row[0]; x.execute("UPDATE public.background_jobs SET status='succeeded',progress=100,result=%s,artifact_id=%s,lease_owner=NULL,lease_expires_at=NULL,completed_at=now() WHERE job_id=%s RETURNING *",(raw,artifact_id,job_id)); updated=x.fetchone()
   if updated is None:raise RuntimeError("failed to complete background job")
 ...
 def artifact(self,job_id:UUID)->JobArtifact|None:
  with self.connection_factory() as c,cursor as x:
