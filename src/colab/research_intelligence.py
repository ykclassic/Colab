"""Phase 8: document ingestion, embeddings, vector retrieval, provenance and citations.

The core is provider-neutral and deterministic by default. Production deployments can
replace the embedding and synthesis implementations without changing the API contract.
"""
...
class DeterministicEmbeddingProvider(EmbeddingProvider):
    """Offline stable embedding used for tests and environments without an AI key.

    This is intentionally a hashing embedding rather than a claim of semantic quality.
    It provides a real vector/cosine-search contract and can be replaced by a hosted
    embedding model without changing stored document or citation contracts.
    """
...
class BasicTextExtractor:
    """Safe dependency-light extractor for text, markdown, JSON and CSV-like input."""
    ...
    def extract(self, content: bytes, media_type: str, filename: str | None = None) -> str:
        ...
        if media_type not in self.allowed_media_types and suffix not in {"txt", "md", "csv", "json"}:
            raise DocumentIngestionError("unsupported document type; Phase 8 currently accepts text, markdown, CSV and JSON")
        try:
...
class InMemoryResearchIntelligenceStore:
    ...
    def list_sources(self, workspace_id: UUID | None = None) -> list[DocumentSource]:
        ...
        document_source_ids = {doc.source_id for doc in self.documents.values() if doc.workspace_id == workspace_id}
        return [source for source in self.sources.values() if source.source_id in document_source_ids]

    def list_documents(self, workspace_id: UUID | None = None) -> list[ResearchDocument]:
        values = self.documents.values()
        if workspace_id is not None:
            values = (doc for doc in values if doc.workspace_id == workspace_id)
        return sorted(values, key=lambda doc: (doc.created_at, str(doc.document_id)))

    def list_chunks(self, document_id: UUID | None = None) -> list[ResearchChunk]:
        values = self.chunks.values()
        if document_id is not None:
            values = (chunk for chunk in values if chunk.document_id == document_id)
...
class ResearchIntelligence:
    ...
    def __init__(
        self,
        store: ResearchIntelligenceStore | None = None,
    ...
    def synthesize(self, query: str, *, limit: int = 8, workspace_id: UUID | None = None) -> ResearchSynthesis:
        ...
        for hit in hits:
            ...
            evidence_lines.append(f"[{citation_id}] {quote}")
        answer = (
            f"Research synthesis for: {query.strip()}\n\n"
