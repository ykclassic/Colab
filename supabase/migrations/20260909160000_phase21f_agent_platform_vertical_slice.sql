...
CREATE POLICY phase21f_agent_benchmarks_select ON public.agent_benchmarks FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.agent_versions v WHERE v.agent_id=agent_benchmarks.agent_id AND v.version=agent_benchmarks.version AND public.is_workspace_member(v.workspace_id))
);
DROP POLICY IF EXISTS phase21f_agent_governance_select ON public.agent_version_governance;
CREATE POLICY phase21f_agent_governance_select ON public.agent_version_governance FOR SELECT TO authenticated USING (
  EXISTS (SELECT 1 FROM public.agent_versions v WHERE v.agent_id=agent_version_governance.agent_id AND v.version=agent_version_governance.version AND public.is_workspace_member(v.workspace_id))
);
REVOKE ALL ON public.agent_versions, public.agent_benchmarks, public.agent_version_governance FROM anon;

CREATE OR REPLACE FUNCTION public.prevent_phase21f_immutable_mutation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'phase21f immutable records cannot be updated or deleted'; END; $$;
...
CREATE OR REPLACE FUNCTION public.validate_phase21f_governance() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.agent_benchmarks b WHERE b.benchmark_hash=NEW.benchmark_hash AND b.agent_id=NEW.agent_id AND b.version=NEW.version) THEN
    RAISE EXCEPTION 'phase21f governance requires matching benchmark';
  END IF;
  RETURN NEW;
END; $$;
